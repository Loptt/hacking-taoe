...
int take_wager(int available_credits, int previous_wager)
{
    ...
    {
        printf("Your total wager of %d is more than you have!", total_wager);
...
void play_the_game()
{
    ...
    while (play_again)
    {
        ...
        if (player.current_game() != -1)
        {
            ...
            //printf("[DEBUG] Game done.\n");
            printf("You now have %u credits\n", player.credits);

            // printf("[DEBUG] Updating player data\n");
            update_player_data();
            // printf("[DEBUG] Done updating\n");
...
int find_the_ace()
{
    ...
    while (invalid_choice)
    {
        ...
        if (choice_two == 'c')
        {
            ...
            pick = i;
            printf("Your card has been changed to card %d\n", pick + 1);
        }
    }
